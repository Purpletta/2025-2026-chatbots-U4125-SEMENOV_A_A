...
```
был получен файл plan.md, который послужил базой для дальнейшего кода
...
We should not have input through /update /add and so on. We should have only buttons. We run the bot, we have no way to write to him we have the opportunity to only click on the button to put the button to click on the habit and after not to click on the habit to create a habit and only after the button to create a habit has worked
